...
if [ "$ACTION" = "dev-start" ]; then
  ...
  docker run -d --name ds_usb_dev --runtime nvidia --network host --privileged -e DISPLAY=:0 -e PYTHONPATH=/app:/app/common -e DS_PGIE_CONFIG=/opt/nvidia/deepstream/deepstream-6.0/samples/configs/deepstream-app/config_infer_primary.txt -e DS_ROS_HOST=127.0.0.1 -e DS_ROS_PORT=9090 -v /tmp/.X11-unix:/tmp/.X11-unix:rw -v /data/ds/share:/app/share -v /data/ds/common:/app/common -v /data/ds/datasets:/data/ds/datasets --device=/dev/video0 deepstream-usb-dev:6.0.1
fi
...
if [ "$ACTION" = "prod-run" ]; then
  export DISPLAY=:0
  xhost +si:localuser:root || xhost +local:root || true
  docker run --rm --runtime nvidia --network host --privileged -e DISPLAY=:0 -e PYTHONPATH=/app:/app/common -e DS_PGIE_CONFIG=/opt/nvidia/deepstream/deepstream-6.0/samples/configs/deepstream-app/config_infer_primary.txt -e DS_ROS_HOST=127.0.0.1 -e DS_ROS_PORT=9090 -v /tmp/.X11-unix:/tmp/.X11-unix:rw -v /data/ds/share:/app/share -v /data/ds/common:/app/common -v /data/ds/datasets:/data/ds/datasets --device=/dev/video0 deepstream-usb-dev:6.0.1 python3 /app/share/deepstream_test_1_usb_ros.py /dev/video0
fi
